...
class FinMyGateTransactionsController extends AppController {
	...
	//var $scaffold;
    protected $base    = "Access Providers/Controllers/FinMyGateTransactions/";
        ...
        $items      = array();
         ...
         foreach($q_r as $i){
            ...
            foreach($this->fields as $field){
                if(array_key_exists($field,$i['FinMyGateTransaction'])){
                    $row["$field"]= $i['FinMyGateTransaction']["$field"];
                }
            }

            //Create notes flag
            $notes_flag  = false;
            foreach($i['FinMyGateTransactionNote'] as $nn){
    ...
    public function delete($id = null) {
	    ...
	    if(isset($this->data['id'])){   //Single item delete
            ...
            $owner_id       = $item['FinMyGateTransaction']['user_id'];
            if($owner_id != $user_id){
	...
	public function view(){
		...
		$q_r   		= $this->FinMyGateTransaction->findById($id);
		//print_r($q_r);
		$data		= array();
		if($q_r){
    ...
    public function edit(){
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->FinMyGateTransaction->save($this->request->data)) {
                   $this->set(array(
                    'success' => true,
                    '_serialize' => array('success')
                ));
    ...
    public function note_index(){
        ...
        if(isset($this->request->query['for_id'])){
            ...
            $q_r    = $this->FinMyGateTransaction->FinMyGateTransactionNote->find('all', 
                array(
                    'contain'       => array('Note'),
                    'conditions'    => array('FinMyGateTransactionNote.fin_my_gate_transaction_id' => $tag_id)
                )
            );
            foreach($q_r as $i){
    ...
    public function note_add(){
        ...
        $this->FinMyGateTransaction->FinMyGateTransactionNote->Note->create(); 
        //print_r($this->request->data);
        if ($this->FinMyGateTransaction->FinMyGateTransactionNote->Note->save($this->request->data)) {
            $d                      = array();
            $d['FinMyGateTransactionNote']['fin_my_gate_transaction_id']   = $this->request->data['for_id'];
            $d['FinMyGateTransactionNote']['note_id'] = $this->FinMyGateTransaction->FinMyGateTransactionNote->Note->id;
            $this->FinMyGateTransaction->FinMyGateTransactionNote->create();
            if ($this->FinMyGateTransaction->FinMyGateTransactionNote->save($d)) {
                $success = true;
            }
        }
    ...
    public function note_del(){
	    ...
	    if(isset($this->data['id'])){   //Single item delete
            ...
            //NOTE: we first check of the user_id is the logged in user OR a sibling of them:   
            $item       = $this->FinMyGateTransaction->FinMyGateTransactionNote->Note->findById($this->data['id']);
            $owner_id   = $item['Note']['user_id'];
            if($owner_id != $user_id){
                if($this->_is_sibling_of($user_id,$owner_id)== true){
                    $this->FinMyGateTransaction->FinMyGateTransactionNote->Note->id = $this->data['id'];
                    $this->FinMyGateTransaction->FinMyGateTransactionNote->Note->delete($this->data['id'],true);
                }else{
                    $fail_flag = true;
                }
            }else{
            ...
            foreach($this->data as $d){
                ...
                $item       = $this->FinMyGateTransaction->FinMyGateTransactionNote->Note->findById($d['id']);
                $owner_id   = $item['Note']['user_id'];
                if($owner_id != $user_id){
                    if($this->_is_sibling_of($user_id,$owner_id) == true){
                        $this->FinMyGateTransaction->FinMyGateTransactionNote->Note->id = $d['id'];
                        $this->FinMyGateTransaction->FinMyGateTransactionNote->Note->delete($d['id'],true);
                    }else{
                        $fail_flag = true;
                    }
                }else{
    ...
    function _build_common_query($user){
        ...
        $sort   = 'FinMyGateTransaction.created';
        $dir    = 'DESC';
        ...
        if($user['group_name'] == Configure::read('group.ap')){  //AP
            ...
            foreach($this->parents as $i){
                ...
                if($i_id != $user_id){ //upstream
                    ...
                    array_push($tree_array,array('FinMyGateTransaction.user_id' => $i_id));
                }
            }
