...
sub dynamic_start{
    ...
    #With this we can specify that a user can only get so much data every 'Rd-Reset-Data-Interval' seconds
    if(exists($RAD_CHECK{'Rd-ResetInterval-Data'})){
        $dyn_start = time-$RAD_CHECK{'Rd-ResetInterval-Data'}; #start in the past
    }
    return $dyn_start;
}
