...
set_nas_id_and_ip(){
    ...
    local config=/etc/chilli/config
    local hb_check=/tmp/heartbeat_check

    #Set HS_NASIP
    eval grep_for_nas_ip=$(grep "^\s*NAS-IP=" $hb_check)
    if [ $grep_for_nas_ip ];
    then
        eval ip=$(echo $grep_for_nas_ip | sed -e "s/^\s*NAS-IP=//g")
        if [ $ip ];
        then
            eval current_ip_line=$(grep "^HS_NASIP=" $config)
            if [ $current_ip_line ];
            then
                eval current_ip=$(echo $current_ip_line | sed -e "s/^\s*HS_NASIP=//g")
                if [ $current_ip != $ip ];
                then
                    sed -i '/^HS_NASIP/d' $config
                    echo ""				>> $config
                    echo "HS_NASIP='$ip'" >> $config     
                fi
            else 
                #Not in file add at the bottom
                echo ""               >> $config                
                echo "HS_NASIP='$ip'" >> $config              
            fi
        fi
    ...
    #Set HS_NASID
    eval grep_for_nas_id=$(grep "^\s*NAS-ID=" $hb_check)
    if [ $grep_for_nas_id ];
	then
        eval id=$(echo $grep_for_nas_id | sed -e "s/NAS-ID=//g")
        if [ $id ];
        then
            eval current_id_line=$(grep "^\s*HS_NASID=" $config)
            echo "1st $current_id_line"
            if [ $current_id_line ];
            then
                eval current_id=$(echo $current_id_line | sed -e "s/^\s*HS_NASID=//g")
                if [ $current_id != $id ];
                then
                    sed -i '/^\s*HS_NASID/d' $config
                    echo ""				>> $config
                    echo "HS_NASID='$id'" >> $config     
                fi
	     else
                 echo ""               >> $config                
                 echo "HS_NASID='$id'" >> $config              
