hostapd_set_bss_options() {
	...
	append "$var" "disassoc_low_ack=$disassoc_low_ack" "$N"
        ...
        if [ -n "$radius_das_client" ]; then
                append "$var" "radius_das_client=$radius_das_client" "$N"
                config_get radius_das_port "$vif" radius_das_port
		[ -z "$radius_das_port" ] && config_get radius_das_port "$vif" radius_das_port
		append "$var" "radius_das_port=$radius_das_port" "$N"
        fi
	local macfile="/var/run/hostapd-${phy}-${ifname}-mac"
        ...
        case "$macfilter" in
			...
			#--- Dirk: They added accounting capabilities but left out the interval setting ---
                        config_get acct_interval "$vif" acct_interval
                                [ -n "$acct_interval" ] && acct_port=${acct_interval:-600}
			...
			test -z "$dynamic_vlan" || {
				...
				config_get vlan_tagged_interface "$vif" vlan_tagged_interface 
				test -z "$vlan_tagged_interface" || append "$var" "vlan_tagged_interface=$vlan_tagged_interface" "$N"
			}
                ;;
        esac
