Ext.define('Rd.controller.cFinAuthorizeNetTransactions', {
    extend: 'Ext.app.Controller',
    actionIndex: function(){

        var me = this;
        var desktop = this.application.getController('cDesktop');
        var win = desktop.getWindow('finAuthorizeNetTransactionsWin');
        if(!win){
            win = desktop.createWindow({
                id: 'finAuthorizeNetTransactionsWin',
                //title: i18n('sProfile_component_manager'),
                btnText: 'Authorize.Net transactions',
                width:800,
                height:400,
                ...
                layout: 'border',
                stateful: true,
                stateId: 'finAuthorizeNetTransactionsWin',
                items: [
                    {
                        region: 'north',
                        xtype:  'pnlBanner',
                        //heading: i18n('sProfile_component_manager'),
                        heading: 'Authorize.Net transaction manager',
                    ...
                    },
                    {
                        ...
                        layout  : 'fit',
                        border  : false,
                        xtype   : 'gridAuthorizeNetTransactions'
                    }
                ]
            });
        }
    ...
    views:  [
        'components.pnlBanner',             'finAuthorizeNetTransactions.gridAuthorizeNetTransactions',
        'components.winCsvColumnSelect',    'components.winNote', 'components.winNoteAdd',
        'finAuthorizeNetTransactions.winAuthorizeNetEmailVoucherDetail'
    ],
    stores: ['sFinAuthorizeNetTransactions', 'sAccessProvidersTree'],
    models: ['mFinAuthorizeNetTransaction',  'mAccessProviderTree' ],
    selectedRecord: null,
    config: {
        urlApChildCheck : '/cake2/rd_cake/access_providers/child_check.json',
        urlExportCsv    : '/cake2/rd_cake/fin_authorize_net_transactions/export_csv',
        urlNoteAdd      : '/cake2/rd_cake/fin_authorize_net_transactions/note_add.json',
        urlEmailSend    : '/cake2/rd_cake/fin_authorize_net_transactions/email_voucher_details.json',
        urlVoucherAttach: '/cake2/rd_cake/fin_authorize_net_transactions/voucher_attach.json',
        urlVoucherDetach: '/cake2/rd_cake/fin_authorize_net_transactions/voucher_detach.json',
    },
    refs: [
        {  ref: 'grid',  selector: 'gridAuthorizeNetTransactions'}       
    ],
    init: function() {
        var me = this;
        if (me.inited) {
        ...
        me.control({
            ...
            'gridAuthorizeNetTransactions #reload': {
                click:      me.reload
            },
            'gridAuthorizeNetTransactions #reload menuitem[group=refresh]' : {
                click:      me.reloadOptionClick
            }, 
            'gridAuthorizeNetTransactions #attach': {
                click:      function(){
                    me.attach_detach('attach');
                }
            },
            'gridAuthorizeNetTransactions #detach': {
                click:       function(){
                    me.attach_detach('detach');
                }
            },
            'gridAuthorizeNetTransactions #email': {
                click:    me.email
            },
            'gridAuthorizeNetTransactions #note'   : {
                click:      me.note
            },
            'gridAuthorizeNetTransactions #csv'  : {
                click:      me.csvExport
            },
            'gridAuthorizeNetTransactions'   : {
                select:      me.select
            },
            'winAuthorizeNetEmailVoucherDetail #send'   : {
                click:      me.emailSend
            },
            'gridNote[noteForGrid=fin_authorize_net_transactions] #reload' : {
                click:  me.noteReload
            },
            'gridNote[noteForGrid=fin_authorize_net_transactions] #add' : {
                click:  me.noteAdd
            },
            'gridNote[noteForGrid=fin_authorize_net_transactions] #delete' : {
                click:  me.noteDelete
            },
            'gridNote[noteForGrid=fin_authorize_net_transactions]' : {
                itemclick: me.gridNoteClick
            },
            'winNoteAdd[noteForGrid=fin_authorize_net_transactions] #btnTreeNext' : {
                click:  me.btnNoteTreeNext
            },
            'winNoteAdd[noteForGrid=fin_authorize_nat_transactions] #btnNoteAddPrev'  : {   
                click: me.btnNoteAddPrev
            },
            'winNoteAdd[noteForGrid=fin_authorize_net_transactions] #btnNoteAddNext'  : {   
    ...
    email: function(button){
        ...
        if(sel_count == 0){
            ...
            if(sel_count > 1){
                ...
                if(!me.application.runAction('cDesktop','AlreadyExist','winEmailAuthorizeNetVoucherDetailId'+sr.getId())){
                    var w = Ext.widget('winAuthorizeNetEmailVoucherDetail',
                        {
                            id          : 'winAuthorizeNetEmailVoucherDetailId'+sr.getId(),
    ...
    note: function(button,format) {
        ...
        if(sel_count == 0){
            ...
            if(sel_count > 1){
                ...
                //Determine the selected record:
                var sr = me.getGrid().getSelectionModel().getLastSelected();    
                if(!me.application.runAction('cDesktop','AlreadyExist','winNoteFinAuthorizeNetTransactions'+sr.getId())){
                    var w = Ext.widget('winNote',
                        {
                            id          : 'winNoteFinAuthorizeNetTransactions'+sr.getId(),
                            noteForId   : sr.getId(),
                            noteForGrid : 'fin_authorize_net_transactions',
            ...
            success: function(response){
                ...
                if(jsonData.success){                      
                    if(jsonData.items.tree == true){
                        if(!me.application.runAction('cDesktop','AlreadyExist','winNoteFinAuthorizeNetTransactionsAdd'+grid.noteForId)){
                            var w   = Ext.widget('winNoteAdd',
                            {
                                id          : 'winNoteFinAuthorizeNetTransactionsAdd'+grid.noteForId,
