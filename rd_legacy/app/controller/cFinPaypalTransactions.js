Ext.define('Rd.controller.cFinPaypalTransactions', {
    extend: 'Ext.app.Controller',
    actionIndex: function(){

        var me = this;
        var desktop = this.application.getController('cDesktop');
        var win = desktop.getWindow('finPaypalTransactionsWin');
        if(!win){
            win = desktop.createWindow({
                id: 'finPaypalTransactionsWin',
                //title: i18n('sProfile_component_manager'),
                btnText: 'Paypal transactions',
                width:800,
                height:400,
                ...
                layout: 'border',
                stateful: true,
                stateId: 'finPaypalTransactionsWin',
                items: [
                    {
                        region: 'north',
                        xtype:  'pnlBanner',
                        //heading: i18n('sProfile_component_manager'),
                        heading: 'Paypal transaction manager',
    ...
    init: function() {
        ...
        me.control({
            ...
            'gridNote[noteForGrid=fin_paypal_transactions] #add' : {
                click:  me.noteAdd
            },
            'gridNote[noteForGrid=fin_paypal_transactions] #delete' : {
                click:  me.noteDelete
            },
            ...
            'winNoteAdd[noteForGrid=fin_paypal_transactions] #btnNoteAddNext'  : {   
            ...
            if(sel_count > 1){
                ...
                if(!me.application.runAction('cDesktop','AlreadyExist','winNoteFinPaypalTransactions'+sr.getId())){
                    var w = Ext.widget('winNote',
                        {
                            id          : 'winNoteFinPaypalTransactions'+sr.getId(),
                            noteForId   : sr.getId(),
                            noteForGrid : 'fin_paypal_transactions',
            ...
            success: function(response){
                ...
                if(jsonData.success){                      
                    if(jsonData.items.tree == true){
                        if(!me.application.runAction('cDesktop','AlreadyExist','winNoteFinPaypalTransactionsAdd'+grid.noteForId)){
                            var w   = Ext.widget('winNoteAdd',
                            {
                                id          : 'winNoteFinPaypalTransactionsAdd'+grid.noteForId,
